...
export async function GET(request) {
  ...
  try {
    ...
    if (cleanBranch.includes("동탄") || cleanBranch.includes("목동") || cleanBranch.includes("오목교") || cleanBranch.includes("대전둔산") || cleanBranch.includes("둔산") || (cleanBranch.includes("대전") && !cleanBranch.includes("유성")) || cleanBranch.includes("강북") || cleanBranch.includes("김포") || cleanBranch.includes("송도") || cleanBranch.includes("인천송도") || cleanBranch.includes("분당정자") || cleanBranch.includes("분당") || cleanBranch.includes("대치") || cleanBranch.includes("이천기숙") || cleanBranch.includes("이천") || cleanBranch.includes("안성기숙") || cleanBranch.includes("안성")) {
      ...
      if ((isDokhakGisuk || isDalseo || isGwangjuDonggu || isChuncheon || isHanam || isIlsandonggu || isIlsanseogu || isMapo || isBusanBukgu) && comp3Keywords.length > 0) {
      ...
      const realTrend = periodMonths.map((period, index) => {
        ...
        if (isDokhakGisuk || isDalseo || isGwangjuDonggu || isChuncheon || isHanam || isIlsandonggu || isIlsanseogu || isMapo || isBusanBukgu) {
      ...
      return NextResponse.json({ success: true, mode: isGwangjuDonggu || isIlsanseogu || isMapo ? "naver-api-4comps" : isDokhakGisuk || isDalseo || isChuncheon || isHanam || isIlsandonggu || isBusanBukgu ? "naver-api-3comps" : "naver-api", trendData: realTrend });
    }
  } catch (error) {
